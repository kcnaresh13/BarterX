<template> 

<div>
<div class="text">
<!--
<p style="text-align:right"> BarterX does allows Hofstra students to trade items of all types with each other. </p> 
-->

<h2>BarterX allows Hofstra students to trade items of all types with each other.  </h2>
<br>

<h2>We provide a money back guarantee if any products are not recieved as advertised. </h2>

<br>
<h3>ACESSORIES </h3>
<img src= "https://m.media-amazon.com/images/I/61u1IR857AL._AC_SS350_.jpg"/>

<h3>TEXTBOOKS </h3>
<img src= "https://encrypted-tbn0.gstatic.com/images?q=tbn%3AANd9GcQKTorzfwB8rhlSHTEEZzkIoT9cDjW8u-KUERwXMozYcw3F7T8vP8yGgPdT0UUD4Ck3prjHVSPt&usqp=CAc"/>
</div>

<br> <br>
</div>




</template>

<script>
export default {

}
</script>

<style>
.text{ font-size: 25 px;
text-align:center;
color: black;
font-family: "Segoe UI", Tahoma;
margin-bottom: 250 px;
}
 h3.text{
    margin-bottom: 50 px;

 }



<template>
  <div>
    <div class="text">
      <!--
<p style="text-align:right"> BarterX does allows Hofstra students to trade items of all types with each other. </p> 
      -->

      <h2>This is the homepage. Our program, BarterX allows Hofstra students to trade items of all types with each other.</h2>
      <br />

      <h2>We provide a money back guarantee if any products are not recieved as advertised.</h2>

      <br />
      <h3>ACESSORIES</h3>
      <img src="https://m.media-amazon.com/images/I/61u1IR857AL._AC_SS350_.jpg" />

      <h3>TEXTBOOKS</h3>
      <img src="../assets/bioTextbook.jpg" />
    </div>
  </div>
</template>

<script>
export default {};
</script>

<style>
.text {
  font-size: 25 px;
  text-align: center;
  color: black;
  font-family: "Segoe UI", Tahoma;
  margin-bottom: 250 px;
}
h3.text {
  margin-bottom: 50 px;
}
</style>